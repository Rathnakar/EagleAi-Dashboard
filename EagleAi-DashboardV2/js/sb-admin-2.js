...
$('#showScatterData').hide();
$('#divisionsid').on('click', '.tradbtn', function() { 
	   $(".tradbtn").toggleClass(function(n){
			  // console.log($("#"+hh).hasClass("btn" + n+"active"));
			   if($("#btn" + (n+1)+"active").hasClass("btn" + (n+1)+"active"))
				   $("#btn" +(n+1)+"active").toggleClass("btn" + (n+1)+"active");
	      return '';
	    }); 
...
$(document).ready(function () {
    var allBoxes = $("div.boxes").children("ul");
...
function transitionBox(from, to) {
    function next() {
        var nextTo;
        if (to.is(":last-child")) {
            nextTo = to.closest(".boxes").children("ul").first();
        } else {
            nextTo = to.next();
        }
