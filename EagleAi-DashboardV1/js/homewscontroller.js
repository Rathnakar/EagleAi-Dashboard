...
function HomewsController($scope,$q,$http,$cookieStore,$location,$window,UserService,$filter,$interval,$routeParams,$uibModal,$rootScope) {
	...
	const divisionSocket = io.connect('http://159.203.112.185:7878/DIVISION');
	const deskSocket = io.connect('http://159.203.112.185:7878/DESK');
	const clientSocket = io.connect('http://159.203.112.185:7878/CLIENT');
	const symbolSocket = io.connect('http://159.203.112.185:7878/SYMBOL');
	const traderSocket = io.connect('http://159.203.112.185:7878/TRADER');
	const omprocessSocket = io.connect('http://159.203.112.185:7878/OMPROCESS');
	const outlierSocket=io.connect('http://159.203.112.185:7879/singleorder_anomaly');
	const divisionIssueSocket = io.connect('http://159.203.112.185:7878/DIVISION_ISSUETYPE');
	const deskIssueSocket = io.connect('http://159.203.112.185:7878/DESK_ISSUETYPE');
	const clientIssueSocket = io.connect('http://159.203.112.185:7878/CLIENT_ISSUETYPE');
	const symbolIssueSocket = io.connect('http://159.203.112.185:7878/SYMBOL_ISSUETYPE');
	const traderIssueSocket = io.connect('http://159.203.112.185:7878/TRADER_ISSUETYPE');
	const omprocessIssueSocket = io.connect('http://159.203.112.185:7878/OMPROCESS_ISSUETYPE');
	$rootScope.restAPIServerName = 'http://localhost:6222'; //'http://ec2-54-191-51-162.us-west-2.compute.amazonaws.com:8222'
		...
		if(lastRead==null){
			angular.forEach(heatData,function(data,index){
				...
				if(data['is_anom']==null || data['is_anom']==false){
					...
					$scope.alertCount++;
					$scope.alerts.push({description:data['heatmapfield'],
							    name:data['heatmapfield'],
				...
				if(data['heatmapfield'].includes(match)){
					$scope.heatSeries.push({name: data['heatmapfield'],value: data['actual'],color:heatColor})
				}
	...
	$scope.generateBarData=function(id,barData,type,division,issueType,dataType,lastRead){
		var filter=["GROSS_NV","NET_NV","NUM_ORDERS"];
		if(lastRead==null){
			angular.forEach(filter,function(f){
				var series= $filter('filter')(barData, { pred_type: f.replace('_'," ") }, true);
	...
	$scope.generateScatterData=function(id,scatterData,type,division,issueType,dataType,divType,lastRead){
		...
		if(lastRead==null){
			angular.forEach(filter,function(f,fIndex){
				var title=type+" "+f.replace('_'," ");
				var result= $filter('filter')(scatterData, { pred_type: f.replace('_'," ") }, true);
				var id=division+"_"+type.toLowerCase()+"_pred_vs_actual_"+f;
				var anomalyCount=0;
				var seriesData=[];
				$scope[division][type.toLowerCase()+"_"+f.replace(' ',"_")]=[];
				...
				angular.forEach(result,function(clientGross,index){
					...
					if(name.match("_")){
						$scope[division][type.toLowerCase()+"_"+f.replace(' ',"_")].push(parseInt(name.split('_')[1]));
					}else{
						$scope[division][type.toLowerCase()+"_"+f.replace(' ',"_")].push(name);
					}
...
name:name,color:scatterColor, marker:{radius: scatterRadius}});
	...
	$scope.getrms=function(lastRead,division,alertData){
		...
		$http.get(restAPIServerName+"/getRMS/"+division+"/"+issueType+"/"+lastRead+"/"+$rootScope.dateFilter).then(function(response){
			...
			angular.forEach(response.data.tsData,function(divTime){
				...
				if(divTime['pred_type'].includes("GROSS NV")){
					divTime['pred_type']=division+"_gross_nv";
				}else if(divTime['pred_type'].includes("NET NV")){
					divTime['pred_type']=division+"_net_nv";
				}else if(divTime['pred_type'].includes("NUM ORDERS")){
			...
			$scope.generateScatterData('Client',response.data.ClientData,'Client',division,issueType,'client',null,lastRead);
			$scope.generateScatterData('Symbol',response.data.SymbolData,'Symbol',division,issueType,'symbol',null,lastRead);
			$scope.generateHeatMapData(response.data.OMPROCESS,'OMPROCESS',division,issueType,'omprocess',lastRead);
			$scope.generateHeatMapData(response.data.TRADER,'Trader',division,issueType,'trader',lastRead);
	...
	$http.get(restAPIServerName+"/").then(function(response){
		angular.forEach(response.data,function(response){
			var division=response.rootvalue;
	...
	*/
	...
	$scope.complianceAnom=function(){
		...
		if(chart==undefined){
			$http.get(restAPIServerName+"/getTs/"+seriesType.toUpperCase()+"/'"+division+"'/"+issueType+"/"+lastRead+"/"+$rootScope.dateFilter+"/"+division).then(function(response){
...
app.controller('indivContentCtrl', function($scope,client,seriesType,division,issueType,$uibModalInstance,$http,$rootScope) {
	...
	$scope.getTimeSeries=function(seriesType,filter,issueType,lastRead){
		$http.get(restAPIServerName+"/getTs/"+seriesType.toUpperCase()+"/'"+filter+"'/"+issueType+"/"+lastRead+"/"+$rootScope.dateFilter+"/"+division).then(function(response){
			$scope.divTimeSeries=response.data;
			angular.forEach($scope.divTimeSeries,function(divTime){
				if(divTime['division']==division){
					var title=seriesType+" "+divTime['pred_type'].replace('_'," ");
